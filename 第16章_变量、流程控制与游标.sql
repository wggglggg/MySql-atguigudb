...
SHOW VARIABLES; 	#不写明global or session,默认查询的是会话session系统变量
...
#方式2：
SET GLOBAL max_connections = 190;
#查看修改后的数值
SELECT @@global.max_connections;

#global针对于当前的数据库实例是有效的，一旦重启mysql服务，就失效了。
...
#session针对于当前会话是有效的，一旦结束会话，重新建立起新的会话，就失效了。
...
BEGIN
	#1、声明局部变量, 可以设置默认初始值，不赋值即为default值
...
BEGIN
	...
	DECLARE hire_years DOUBLE; #记录员工入职公司的年头
	
	#赋值
	SELECT salary INTO emp_sal FROM employees WHERE employee_id = emp_id;
	
	SELECT DATEDIFF(CURDATE(), hire_date)/365 INTO hire_years FROM employees WHERE employee_id = emp_id;
	
	#判断
	IF emp_sal < 8000 AND hire_years >= 5
...
BEGIN
	...
	#判断
	IF emp_sal < 9000 AND hire_years >= 5
		THEN UPDATE employees SET salary = salary + 500 WHERE employee_id = emp_id;
	ELSE
...
BEGIN
	...
	DECLARE com_pct DOUBLE; 	   #记录员工的奖金率
	...
	ELSEIF emp_sal < 10000 AND com_pct IS NULL
...
SELECT * FROM employees WHERE employee_id IN (102,103, 104, 105);
...
BEGIN
	...
	*/
	...
	CASE
		WHEN var1 >= 100 THEN SELECT '过百';
		WHEN var1 >= 10 THEN SELECT '过十小百';
		ELSE SELECT '个位数';
	END CASE;
...
BEGIN
	#声明局部变量
	DECLARE hire_years INT;  #记录员工入职公司的总时间（单位：年）
	
	
	#赋值
	SELECT DATEDIFF('1995-1-1', hire_date)/365 INTO hire_years
...
BEGIN
	...
	DECLARE num_count INT DEFAULT 0; #记录循环的次数
...
BEGIN
	#声明变量
	DECLARE avg_salary DOUBLE; 		#记录平均工资
	
	DECLARE num_count INT DEFAULT 0;	#记录循环次数
...
BEGIN
	#声明局部变量
	DECLARE works_count INT DEFAULT 0; #记录累加的人数
