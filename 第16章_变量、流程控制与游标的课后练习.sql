...
BEGIN
	#声明变量
	DECLARE init_num INT DEFAULT 1; #①初始化条件
	
	WHILE init_num <= insert_count DO
		#③ 循环体
		INSERT INTO admin(user_name, user_pwd) 
		VALUES(CONCAT('Number', init_num), ROUND(RAND()*1000000));
...
BEGIN
	...
	DECLARE emp_count INT DEFAULT 0;#记录涨薪员工个数
...
FROM employees
WHERE department_id = 50 AND employee_id IN (132,127,135,128)
ORDER BY salary ASC;
...
SELECT 2310, 2400 , 2400, 2420
SELECT 2541, 2640, 2640
