...
FROM employees
#where salary = 6000;
WHERE commission_pct = NULL;  #此时执行，不会有任何的结果,不要与null作简单的运算
...
SELECT 3 <> 2,'4' <> NULL, '' != NULL,NULL != NULL           #不要与Null做简单运算
FROM DUAL;
...
#② LEAST() \ GREATEST  最小与最大
SELECT LEAST('g','b','t','m'),GREATEST('g','b','t','m')  # b  t
FROM DUAL;

SELECT LEAST(last_name, first_name), first_name, last_name
FROM employees;
...
SELECT 'Konka' REGEXP '[a | b]', 'Hisenses' REGEXP 'si'
FROM DUAL;
