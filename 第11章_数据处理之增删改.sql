...
(1, 'Tom', '2023-01-21', 3400); #注意：一定要按照声明的字段的先后顺序添加
#错误的
INSERT INTO emp1
VALUES (2,3400,'2000-12-21','Jerry');

# ② 指明要添加的字段 （推荐）
INSERT INTO emp1 (id, hire_date, salary, `name`) VALUES
...
SELECT *
FROM books
LIMIT 5, 5; #从index 5(第6本)开始显示, 每页显示5本
...
#REPLACE(`name`, ' ', '') 将书名中空格去除
#CHAR_LENGTH(str) 计算字符的长度
SELECT `name`, note
FROM books
...
# 优化
SELECT IFNULL(note, '合计') "种类", SUM(num) "汇总"
FROM books
GROUP BY note WITH ROLLUP;
...
# 优化
SELECT IFNULL(note, '合计') "种类", COUNT(*) "分类汇总"
FROM books
GROUP BY note WITH ROLLUP;
