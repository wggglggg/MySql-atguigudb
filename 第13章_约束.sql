...
email VARCHAR(25),
salary DECIMAL(10,2),
#表级约束 uk为unique的简写，uk_test2_email 见名知意
...
cid INT,		#课程编号
score INT,		#学分
UNIQUE KEY(sid, cid)	#复合唯一 两个值 都相同才触发unique Key
);
...
#如何删除唯一性索引
ALTER TABLE test2
DROP INDEX last_name; #删除last_name唯一性约束

ALTER TABLE test2
DROP INDEX uk_test2_sal; #删除uk_test2_sal工资唯一性约束
...
#错误：Multiple primary key defined创建了多个主键Primary key
CREATE TABLE test3(
...
#错误：Duplicate entry '1' for key 'test4.PRIMARY' 重复id
INSERT INTO test4
VALUES(1, 'Tom', 4500, 'tom@126.com');


#错误：Column 'id' cannot be null id为null
INSERT INTO test4(id,last_name,salary,email)
...
#错误：Column 'name' cannot be null 主键不能为null
INSERT INTO user1
VALUES(1, NULL, 'abc');
...
INSERT INTO test9
VALUES(0);		#删除id4后再添加一条数据 ，id4不再重复
...
INSERT INTO test9
VALUES(0);		#重启服务器，再添加一条数据 ，id5会重复添加

#在MySQL 8.0中演示，重启服务器后再添加，不会出现之前使用过的id
CREATE TABLE test9(
id INT PRIMARY KEY AUTO_INCREMENT
);
...
#操作报错，因为主表中的dept_id上没有主键约束或唯一性约束。
CREATE TABLE emp1(
...
#7.2 演示外键的效果
#添加失败,因为dept1部门表没有10号部门信息
INSERT INTO emp1
VALUES(1001, 'Tom', 10);

#要先将部门表添加一条信息，才可以再emp1员工表中添加10号部门的员工
INSERT INTO dept1
VALUES(10, 'IT');
...
#删除失败,因为与emp1员工表有关系，无法直接删除部门表dept1
DELETE FROM dept1
WHERE dept_id = 10;

#更新失败 为与emp1员工表有关系，无法更新部门表dept1
UPDATE dept1
SET dept_id = 20
WHERE dept_id = 10;
...
#演示：
# on update cascade on delete set null 建议此方式 更新主表数据时，子表自动更新，删除主表数据时，子表自动设改null
CREATE TABLE dept(
...
#删除了外键，一般还会留下索引，再手动的删除外键约束对应的普通索引
...
#错误：Check constraint 'test10_chk_1' is violated. 限制了最低金额
INSERT INTO test10
VALUES(2, 'Jim', 1500);
