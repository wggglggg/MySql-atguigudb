...
#查看创建表的语句结构
SHOW CREATE TABLE myemp1;  # utf8mb3  如果创建表时没有指明使用的字符集，则默认使用表所在的数据库的字符集。
#查看表数据
SELECT * FROM myemp1;
...
ALTER TABLE myemp1
ADD phone_number VARCHAR(20) FIRST;  #添加到表中第一个字段的位置

ALTER TABLE myemp1
ADD email VARCHAR(45) AFTER emp_name;  #添加到表中emp_name字段的后面位置
...
ALTER TABLE myemp1
MODIFY emp_name VARCHAR(35) DEFAULT 'new one';
...
#方式2:
RENAME TABLE myemp2 TO myemp12;
...
FROM myemp3;
#3)
SET autocommit = FALSE;		# 将autocommit自动提交改为false,意为手动commit
#4)
DELETE FROM myemp3;		# 删除myemp3表中的所有数据 
#5)
SELECT *
FROM myemp3;
#6)
ROLLBACK;			# 回滚前一些commit时的状态
#7)
SELECT *
FROM myemp3;			# 数据成功恢复
...
SET autocommit = FALSE;		# 将autocommit自动提交改为false,意为手动commit
#4)
TRUNCATE TABLE myemp3;		# 删除myemp3表中的所有数据
#5)
SELECT *
FROM myemp3;
#6)
ROLLBACK;			# 回滚前一些commit时的状态, 回滚失败
#7)
SELECT *
FROM myemp3;			# 数据恢复失败, truncate是无法回滚的
...
DROP TABLE book1, book2;	# mysql8.0新特性,同时删除多张表,有一张表不存在,那么所有表都不会被删除

SHOW TABLES;			# mysql5.4中,同时删除多张表,有一张表不存在,那么会删除已经存在的表